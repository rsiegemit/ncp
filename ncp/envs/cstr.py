...
class CSTREnv(BaseEnv):
    """2-D Continuous Stirred Tank Reactor environment.

    State: ``[C_A, T]``, control: heat input ``Q``.
    No angular wrapping.

    Args:
        num_envs: Number of parallel environments.
        dynamics_fn: Override dynamics (default: ``cstr_dynamics``).
        max_heat: Symmetric heat input bound.
    ...
    def __init__(
        ...
        *,
        ...
        control_spec = ControlSpec(
            dim=1,
            lower_bounds=torch.tensor([-max_heat]),
            upper_bounds=torch.tensor([max_heat]),
        )
